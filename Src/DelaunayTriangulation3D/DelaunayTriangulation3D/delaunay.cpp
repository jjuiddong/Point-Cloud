...
const std::vector<Triangle2>& Delaunay::triangulate(std::vector<Vector3> &vertices)
{
	// Store the vertices localy
	_vertices = vertices;

	// Determinate the super Triangle2
	...
	Vector3 p1(midx - 20 * deltaMax, midy - deltaMax, 0);
	Vector3 p2(midx, midy + 20 * deltaMax, 0);
	Vector3 p3(midx + 20 * deltaMax, midy - deltaMax, 0);

	//std::cout << "Super Triangle2 " << std::endl << Triangle2(p1, p2, p3) << std::endl;
	
	// Create a list of Triangle2s, and add the superTriangle2 in it
	_triangles.push_back(Triangle2(p1, p2, p3));
	...
	for(auto p = begin(vertices); p != end(vertices); p++)
	{
		...
		_triangles.erase(std::remove_if(begin(_triangles), end(_triangles), [badTriangles](Triangle2 &t){
		...
		for(auto e = begin(polygon); e != end(polygon); e++)
			_triangles.push_back(Triangle2(e->p1, e->p2, *p));
	
	}

	_triangles.erase(std::remove_if(begin(_triangles), end(_triangles), [p1, p2, p3](Triangle2 &t){
