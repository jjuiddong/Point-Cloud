...
void cParticles::Render(cRenderer &renderer)
{
	...
	renderer.GetDevice()->SetRenderState(D3DRS_POINTSPRITEENABLE, TRUE);       // Turn on point sprites
	renderer.GetDevice()->SetRenderState(D3DRS_POINTSCALEENABLE, TRUE);       // Allow sprites to be scaled with distance
	renderer.GetDevice()->SetRenderState(D3DRS_POINTSIZE, FtoDW(m_pointSize)); // Float value that specifies the size to use for point size computation in cases where point size is not specified for each vertex.
	renderer.GetDevice()->SetRenderState(D3DRS_POINTSIZE_MIN, FtoDW(1.0f));    // Float value that specifies the minimum size of point primitives. Point primitives are clamped to this size during rendering. 
	renderer.GetDevice()->SetRenderState(D3DRS_POINTSCALE_A, FtoDW(0.0f));    // Default 1.0
	renderer.GetDevice()->SetRenderState(D3DRS_POINTSCALE_B, FtoDW(0.0f));    // Default 0.0
	renderer.GetDevice()->SetRenderState(D3DRS_POINTSCALE_C, FtoDW(1.0f));    // Default 0.0
	...
	m_vtxBuffer.RenderPointList(renderer, m_particleCount);

	m_texture->Unbind(renderer, 0);
